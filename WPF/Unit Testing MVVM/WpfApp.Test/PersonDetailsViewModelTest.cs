...
namespace WpfApp.Test
{
	...
	public class PersonDetailsViewModelTest : TestBase
	{
		...
		public void PersonDetailsViewModel_GivenEmptyViewModel_WhenNewPersonCommandIsExecuted_ThenPersonDialogShouldBeDisplayed()
		{
			...
			dialogServiceMock.Verify(x => x.ShowDialog<PersonDialog>(
                It.Is<object>(o => object.ReferenceEquals(o, viewModel)), It.IsAny<object>()), Times.Once, 
                "New person dialog was not displayed.");
		}
		...
		public void PersonDetailsViewModel_GivenEmptyViewModel_WhenNewPersonWasCreatedSuccessfully_ThenDirectoryUpdatedEventShouldBePublished()
		{
			...
			dialogServiceMock.Setup(x => x.ShowDialog<PersonDialog>(It.IsAny<object>(), 
                It.IsAny<object>())).Returns(true);

			viewModel.NewPersonCommand.Execute(null);

			personDirectoryUpdatedEventMock.Verify(x => x.Publish(It.IsAny<object>()), Times.Once, 
                "Person directory updated event was not published");
		}
		...
		public void PersonDetailsViewModel_GivenEmptyViewModel_WhenNewPersonWasNotCreatedSuccessfully_ThenDirectoryUpdatedEventShouldNotBePublished()
		{
			...
			personDirectoryUpdatedEventMock.Verify(x => x.Publish(It.IsAny<object>()), 
                Times.Never, "Person directory updated event was published, but it shouldn't have been");
		}
